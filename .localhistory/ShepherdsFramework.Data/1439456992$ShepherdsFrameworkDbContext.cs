...
namespace ShepherdsFramework.Data
{
    ...
    public class ShepherdsFrameworkDbContext : DbContext, IDbContext
    {
        ...
        {
            var typesToRegister = Assembly.GetExecutingAssembly().GetTypes().Where(type => !String.IsNullOrEmpty(type.Namespace)).Where(type =>type.BaseType != null && type.BaseType.IsGenericType && type.BaseType.IsGenericTypeDefinition==typeof(EntityTypeConfiguration<>))
