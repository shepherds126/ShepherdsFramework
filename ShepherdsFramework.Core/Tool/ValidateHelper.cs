...
namespace ShepherdsFramework.Core.Tool
{
    ...
    {
        //邮件正则表达式
        private static Regex _emailregex = new Regex(@"^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$");

        //手机号正则表达式
        private static Regex _mobileregex = new Regex(@"^1\d{10}$");

        //固话号正则表达式
        private static Regex _phoneregex = new Regex(@"(^(0\d{2})-(\d{8})$)|(^(0\d{3})-(\d{8})$)|(^(0\d{3})-(\d{7})$)|(^(0\d{2})-(\d{8})-(\d+)$)|(^(0\d{3})-(\d{8})-(\d+)$)|(^(0\d{3})-(\d{7})-(\d+)$)");
        ...
        //正整数正则表达式
        private static Regex _numberregex = new Regex(@"^[0-9]*[1-9][0-9]*$");

        //订单号验证
        private static Regex _ordernoregex = new Regex(@"^\d{16}$");

        //只能包含中文、英文、数字、下划线等字符
        private static Regex _stringcheckregex = new Regex(@"^[\u4e00-\u9fa5\w]+$");
        /// <summary>
        /// (密码1)字母、数字或符号两种以上组合
        /// </summary>
        private static Regex _password1checkregex = new Regex(@"^(?![A-Z]+$)(?![a-z]+$)(?!\d+$)(?![\W_]+$)\S+$");
        /// <summary>
        /// (密码2)字母组合
        /// </summary>
        private static Regex _password2checkregex = new Regex(@"^[A-Za-z]+$");
        /// <summary>
        /// 是否是正整数
        /// </summary>
        ...
        /// <param name="s"></param>
        /// <returns></returns>
        public static bool IsPasswordCheck(string s)
        {
            return _password1checkregex.IsMatch(s) || _password2checkregex.IsMatch(s);
        }

        /// <summary>
        /// 验证字符串是否只包含中文、英文、数字、下划线等字符
        /// </summary>
        /// <param name="s"></param>
        /// <returns></returns>
        public static bool IsStringCheck(string s)
        {
            if (string.IsNullOrEmpty(s)) return true;
            return _stringcheckregex.IsMatch(s);
        }
        ...
        /// <returns>是则返回true;否则,返回false</returns>
        public static bool CheckOrderNo(string orderno)
        {
            if (string.IsNullOrEmpty(orderno)) return false;
            return _ordernoregex.IsMatch(orderno);
        }
    }
}
